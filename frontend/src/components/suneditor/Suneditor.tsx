...
import {events, uploadBeforeEvents} from '@/components/suneditor/data/events'
import getLanguage from '@/components/suneditor/lang/getLanguage'
import {type SunEditorReactProps} from '@/components/suneditor/types/SunEditorReactProps'

export const SunEditor: FC<SunEditorReactProps> = (props) => {
  const {
    name,
    lang,
    setOptions = {},
  ...
  useEffect(() => {
    ...
    editor.current.onload = (_, reload): any => {
      ...
      if (editor.current!.util.isIE) (editor.current!.core as any)._createDefaultRange()
      ...
      if (autoFocus === false) (editor.current!.core.context.element.wysiwyg as any).blur()
      else if (autoFocus) (editor.current!.core.context.element.wysiwyg as any).focus()
    ...
    if (onBlur) {
      editor.current.onBlur = (e) => onBlur(e, editor.current!.getContents(true))
    }
    ...
    uploadBeforeEvents.forEach((event) => {
      ...
      if (editor.current && value)
        editor.current[event] = (files: Array<File>, info: object, _: any, uploadHandler: any) =>
  ...
  useEffect(() => {
    if (setDefaultStyle && !initialEffect.current) editor.current?.setDefaultStyle(setDefaultStyle)
  }, [setDefaultStyle])

  useEffect(() => {
    if (!initialEffect.current && setContents !== undefined && !editor.current?.core.hasFocus) {
      editor.current?.setContents(setContents)
    }
  }, [setContents])

  useEffect(() => {
    if (!initialEffect.current && appendContents !== undefined && !editor.current?.core.hasFocus) {
  ...
  return <textarea style={{visibility: 'hidden'}} ref={txtArea} {...{name}} />
}
